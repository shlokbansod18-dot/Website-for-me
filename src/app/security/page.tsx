...
const PILLARS = [
  ...
  },
  {
    ...
    body: "A card number exists as a local variable for the instant a payment is authorised, then it is gone. It is never written to the database, never logged, and never included in an error message. All that survives is the brand and the last four digits, so your receipt can say “Visa ending 4242”.",
  },
  {
    glyph: "🔒",
    ...
    <>
        ...
        <div className="shell relative py-20 lg:py-28">
          <div className="max-w-3xl">
            ...
            <h1 className="display mt-6">
              Boring
              <br />
              <span className="text-acid">on purpose.</span>
            </h1>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {PILLARS.map((pillar, i) => (
            <article
              ...
              className="card p-7 transition-colors hover:border-line-strong"
            >
              <span aria-hidden className="text-2xl">
                {pillar.glyph}
              </span>
              <h2 className="mt-5 font-display text-lg font-bold leading-snug tracking-[-0.035em]">
                {pillar.title}
              </h2>
              <p className="mt-3 text-[0.875rem] leading-relaxed text-dim">{pillar.body}</p>
            </article>
          ))}
        </div>
      </section>
        ...
        <div className="shell grid gap-12 lg:grid-cols-[1fr_1.3fr] lg:items-start">
          ...
          <ul className="space-y-px overflow-hidden rounded-[1.5rem] border border-line">
            {PROMISES.map((promise) => (
              <li key={promise} className="flex gap-4 bg-canvas px-7 py-5 text-[0.875rem]">
                <span aria-hidden className="mt-0.5 shrink-0 text-acid">
                  ✓
                </span>
                <span className="text-dim">{promise}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <section className="shell py-20">
        <div className="card mx-auto max-w-3xl p-8 sm:p-10">
          ...
          <p className="mt-4 text-[0.875rem] leading-relaxed text-dim">
            This project ships with a self-contained demo payment processor so the whole flow works
            out of the box. It is written the way a real integration should be — the card number
          ...
          <p className="mt-4 text-[0.875rem] leading-relaxed text-dim">
            ...
            Lemon Squeezy. Their iframe collects the card so the number never reaches your server at
            all, which keeps you outside PCI-DSS scope entirely. Only the body of{" "}
            <code className="rounded bg-surface-2 px-1.5 py-0.5 font-mono text-xs text-acid">
              charge()
            </code>{" "}
            needs to change.
          </p>
        ...
        <p className="mx-auto mt-8 max-w-3xl text-center text-[0.6875rem] leading-relaxed text-faint">
          Found something wrong? Tell us before you tell anyone else — see{" "}
          <Link href="/help" className="underline underline-offset-2 hover:text-dim">
